...
The `git push` will fail if you're not up-to-date: for instance, if
...
Suppose you're embarking on a long-term project, like working on a new
...
A `git clone` only sets up one local branch, `master`, which tracks the
